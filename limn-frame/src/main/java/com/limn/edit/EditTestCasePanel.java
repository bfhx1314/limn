...
//		executeTestCase.setMargin(new Insets(0,0,0,0));
		...
		insert.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (name != null) {
					...
					moduleKey.setText(moduleJList.getSelectedValue().toString());
				}
			}
		});
	...
	private void setTestCaseByModule(int moduleIndex){
		if(isRefreshTable){
			clearTable();
			String[][] testCaseStep = eTestCase.getModuleTestCaseByIndex(moduleIndex);
	...
	private String openFile(String title,boolean isOpen){
		...
		}else if(new File(Parameter.TMP + "/userlike/openFilePath.txt").exists()){
			fileName = new File(readFile(Parameter.TMP + "/userlike/openFilePath.txt"));
			fileChooser.setSelectedFile(fileName);
		}
	...
	private void deleteRow(int index){
		...
		if(testCaseModel.getRowCount()!=0 && testCaseModel.getRowCount()>index){
			testCaseTable.setRowSelectionInterval(index, index);
		}else if(testCaseModel.getRowCount()!=0 && testCaseModel.getRowCount()<=index){
...
//				this.setText(keyEvent);
	...
	class TableCellCheckBoxRenderer extends JCheckBox implements TableCellRenderer {
...
//				setText(value.toString());
				...
				//取行高度 * 行数
//				System.out.println(this.getRows() +" : "+  this.getLineCount());
