...
public class EditTestCasePanel extends CustomPanel {
	...
	public EditTestCasePanel() {
		...
		testCaseTable.setDefaultRenderer(Object.class,
				new TableCellTextAreaRenderer());
		testCaseTable.setDefaultEditor(Object.class,
				new TableCellTextAreaEdit());

		testCaseTable.getColumnModel().getColumn(0)
				.setCellEditor(new TableCellCheckBoxEdit());
		testCaseTable.getColumnModel().getColumn(0)
				.setCellRenderer(new TableCellCheckBoxRenderer());

		testCaseTable.setColumnModel(getColumn(testCaseTable, new int[] { 20,
				20, 20, 200, 150 }));

		testCaseTable.putClientProperty("terminateEditOnFocusLost",
				Boolean.TRUE);
	...
	private void saveModuleCase() {
		if (moduleJList.getSelectedIndex() != -1) {
			...
			for (int i = 0; i < rowCount; i++) {
				testCase[i][0] = testCaseTable.getValueAt(i, 0) == null ? ""
						: testCaseTable.getValueAt(i, 0).toString();
				testCase[i][1] = testCaseTable.getValueAt(i, 1) == null ? ""
						: testCaseTable.getValueAt(i, 1).toString();
				testCase[i][2] = testCaseTable.getValueAt(i, 2) == null ? ""
						: testCaseTable.getValueAt(i, 2).toString();
				testCase[i][3] = testCaseTable.getValueAt(i, 3) == null ? ""
						: testCaseTable.getValueAt(i, 3).toString();
				testCase[i][4] = testCaseTable.getValueAt(i, 4) == null ? ""
						: testCaseTable.getValueAt(i, 4).toString();
			}
